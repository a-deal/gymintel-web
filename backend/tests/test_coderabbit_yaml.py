    ...
    def test_boolean_field_validation(self):
        ...
        for section, field in boolean_fields:
            if section in parsed and field in parsed[section]:
                ...
                assert isinstance(value, bool), f"{section}.{field} should be boolean, got {type(value)}"
    ...
    def test_language_configuration_validation(self):
        ...
        if 'language' in parsed:
            for lang, config in parsed['language'].items():
                assert isinstance(config, dict), f"Language {lang} config should be a dict"
                if 'enable_review' in config:
                    assert isinstance(config['enable_review'], bool), f"{lang}.enable_review should be boolean"
            ...
            for key, expected_type in schema.items():
                if key in data:
                    if isinstance(expected_type, dict):
                        ...
                        assert isinstance(data[key], expected_type), f"{path}.{key} should be {expected_type}"
...
""")
        ...
        # Common language codes that should be supported
        valid_languages = ['python', 'javascript', 'typescript', 'java', 'cpp', 'c', 'go', 'rust']
        
        for lang in config.get('language', {}):
            # This is a simple validation - in real implementation you might have a whitelist
