...
class GymResolvers:
    ...
    async def search_gyms(
        ...
        async with get_db_session() as session:
            ...
            query = select(Gym).options(
                selectinload(Gym.sources),
                selectinload(Gym.reviews)
            )
            ...
            if existing_gyms:
                return SearchResult(
                    ...
                    yelp_results=sum(1 for gym in existing_gyms if any(s.name == "Yelp" for s in gym.sources)),
                    google_results=sum(1 for gym in existing_gyms if any(s.name == "Google Places" for s in gym.sources)),
                    merged_count=sum(1 for gym in existing_gyms if len(gym.sources) > 1),
                    avg_confidence=sum(gym.confidence for gym in existing_gyms) / len(existing_gyms) if existing_gyms else 0.0,
        ...
        try:
            ...
            for gym_data in gyms_data:
                gym = GymType(
                    id=str(gym_data.get("id", f"cli-{hash(gym_data['name'] + gym_data['address'])}")),
                    name=gym_data["name"],
            ...
            return SearchResult(
                ...
                timestamp=datetime.fromisoformat(search_info["timestamp"]) if search_info.get("timestamp") else datetime.utcnow(),
                gyms=gyms,
                ...
                avg_confidence=float(search_info["avg_confidence"].replace("%", "")) / 100.0 if isinstance(search_info["avg_confidence"], str) else search_info["avg_confidence"],
    ...
    async def gyms_by_metro(
        ...
        async with get_db_session() as session:
            query = select(Gym).options(
                ...
                Gym.metropolitan_area_code == metro_code
    ...
    async def gym_analytics(zipcode: str) -> GymAnalytics:
        ...
        async with get_db_session() as session:
            ...
            confidence_hist = {"0.0-0.2": 0, "0.2-0.4": 0, "0.4-0.6": 0, "0.6-0.8": 0, "0.8-1.0": 0}
            ...
            return GymAnalytics(
                ...
                market_saturation="high" if total_gyms > 20 else "medium" if total_gyms > 10 else "low"
            )
    
    @staticmethod
    async def market_gap_analysis(zipcode: str, radius: float = 10.0) -> List[MarketGap]:
